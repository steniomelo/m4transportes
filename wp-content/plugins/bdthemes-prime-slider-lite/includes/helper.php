...
function prime_slider_get_category($terms, $cached = true) {

    $data = get_transient( 'ps_get_category_' . $terms );
    ...
    if ( false === $data ) {
        ...
        if ( true == $cached ) {
            set_transient( 'ps_get_category_' . $terms, $post_options, MINUTE_IN_SECONDS );
            $data = get_transient( 'ps_get_category_' . $terms );
        } else {
            $data = $post_options;
        }
